...
This emulation plan contains several placeholder values that are meant to be replaced with values specific to the target environment against which this plan is to be run. For ease of use, a script has been included to automatically make these substitutions, found [here](placeholder_substitution1.). 
...
## Pre-requisites
Prior to beginning the following emulation Scenario, ensure you have the proper infrastructure requirements and configuration in place as stated in the [Scenario 1 Infrastructure](Infrastructure5.md) documentation.
...
`Adb156.exe` then loads `scrobj.dll` and executes [`sql-rat.js`](sql-rat.js) via jscript ([T1059.7](https://attack.mitre.org/techniques/T1059/007/)).
...
1. Upload the [PowerShell stager](stager.ps1) via SQLRat.
    ```
