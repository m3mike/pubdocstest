...
The [Resources Folder](wizard_spider_Resources.) contains the emulated software source code. Executables are provided in password protected zip files located in the specified software folder.  The password is `malware`.

The [Binaries.zip](Binaries.) contains all executables in one zip file for easy download. The password is `malware`.

We provide a [script](Resources_utilities_crypt_executables.py) to automatically decrypt these files:
...
[YARA rules](wizard_spider_yara-rules.) are provided to assist the community in researching, preventing, and detecting malware specimens used in this emulation plan.
...
- [Detection Scenario](wizard_spider_Emulation_Plan_Scenario_1.) - Step by Step walkthrough of Scenario's procedures (10 steps). 
- [Protection Scenario](wizard_spider_Emulation_Plan_Scenario_2.) - Step by Step walkthrough of Scenario's procedures (6 tests)

## For Analysts 🔎
- [Operation Flow](wizard_spider_Operations_Flow_Operations_Flow.md) - High-level summary of the scenario & infrastructure with diagrams. 
- [Intelligence Summary](wizard_spider_Intelligence_Summary_Intelligence_Summary.md) - General overview of the Adversary with links to reporting used throughout the scenario. 
